...
async fn run_daemon(root: PathBuf, config_dir: Option<&Path>) -> AgentdResult<()> {
    ...
    if std::env::var("WARP_AGENTD_RUN_ONCE").ok().as_deref() == Some("1") {
...
fn usage_message() -> &'static str {
    concat!(
        "Usage:\n",
        "  warp-agentd [--config-dir <path>]\n",
        "  warp-agentd help\n",
        "  warp-agentd init-config [--stdout] [--config-dir <path>]\n",
        "\n",
        "Commands:\n",
        "  help                 Show this help message.\n",
        "  init-config          Initialize config directory warp-agentd/ and write agentd.toml.\n",
...
fn config_file_exists(config_root: &Path) -> bool {
    config_root.join(AGENTD_CONFIG_FILE).is_file()
