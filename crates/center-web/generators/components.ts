...
function chineseLabel(name: string): string | null {
  ...
  const labels: Record<string, string> = {
    ...
    GatewayVersionText: "网关版本",
    GatewayOnlineStatusBadge: "在线状态",
    GatewayHealthBadge: "健康状态",
...
function pageTitle(name: string): string {
  ...
  if (/GatewayList(Page|Screen)$/.test(name)) return "网关列表";
  if (/GatewayInstance(Page|Screen)$/.test(name)) return "网关实例";
  if (/GatewayConfig(Page|Screen)$/.test(name)) return "网关初始配置";
  if (/Release(Page|Screen)$/.test(name)) return "版本发布";
  if (/UpgradePlan(Page|Screen)$/.test(name)) return "升级计划";
...
function pageSummary(name: string): string {
  ...
  if (/GatewayList(Page|Screen)$/.test(name)) {
    return "查看各 WarpGateWay 实例的在线状态、版本、健康状态与最后上报时间，形成全局网关视图。";
  }
  if (/GatewayInstance(Page|Screen)$/.test(name)) {
    return "创建 GateWay 管理实例并为实例绑定客户 ID，建立实例与客户的归属关系。";
  }
  if (/GatewayConfig(Page|Screen)$/.test(name)) {
